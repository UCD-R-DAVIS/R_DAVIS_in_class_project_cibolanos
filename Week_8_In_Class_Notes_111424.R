...
pg <- gapminder %>% 
  ...
  filter(year > 2000) %>% #the only years greater than 2000 is 2002 and 2007
  pivot_wider(names_from = year, values_from = pop) %>% 
  mutate(pop_change_0207 = `2007` - `2002`) #the back tick tells R its a column name, you also have to use it if your column name has a space (like Last Name)

#data visualization
pg %>% 
  filter(continent != "Oceania") %>% 
  ggplot(aes(x = reorder(country, pop_change_0207), y = pop_change_0207)) + #order the countries to least to greatest population change
  geom_col(aes(fill = continent)) + #if color is used it will outline the bars but not fill it
  facet_wrap(~continent, scales = "free") +
  theme_bw() +
  scale_fill_viridis_d() + #can also use scale_fill_brewer(palette = "Set2")
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position = "none") +
  xlab("Country") +
  ylab("Change in Population Between 2002 and 2007")
 #you can add geom_text(size=0.5) after geom_col and label = pop_change_207 inside the ggplot text to have the y-values right on top
# to have an outline box you add color="" after the fill=continent part but after it with a comma like 
#geom_col(aes(fill=continent),color="black")
#adding coord_flip() at the bottom flips your axis without you having to rewrite everything
...
sample_dates_3<- as.Date(sample_dates_2, format = "%m-%d-%Y" ) #Y is for 4 digit year
...
# subtract dates seq(mdy('01-01-2020'),mdy('10-01-2020'), by=1) 
# another way to subtract dates  mdy('01-01-2020')-months(2)

# decimal_date(mdy('10-01-2020')) creates a decimal date of your data in case stuff is not converting right
...
f_to_K <- function(tempF){
  ((tempF - 32) * (5/9)) + 273.15 #tempF will be the argument
} 
